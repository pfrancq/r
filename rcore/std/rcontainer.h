...
public:
	...
	*/
	void Clear(unsigned int m,unsigned int i=0);
	...
	*/
	void InsertPtrOrderAt(const C* ins,unsigned int pos) {BasicContainer::InsertPtrOrderAt(static_cast<const void*>(ins),pos);}
	...
	*/
	void InsertPtrAt(const C* ins,unsigned int pos,bool del=bAlloc);
	...
	* pointers).
	*/
	inline size_t GetTab(C** tab) {return(BasicContainer::GetTab(reinterpret_cast<void**>(tab)));}
	...
	* pointers).
	*/
	inline size_t GetTab(const C** tab) const {return(BasicContainer::GetTab(reinterpret_cast<const void**>(tab)));}
