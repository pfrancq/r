...
protected:
	...
	RPoint Attraction;
	/** Array representing the different levels when using a heuristic BottomLeft
		* or Edge.*/
	RPoint *Levels;
	/** Number of Levels.*/
...
public:
  ...
  RChromo2D(cInst *inst,unsigned int id) throw(bad_alloc);
	...
	/** This function constructs a set of objects that are not already selected
		* for a crossover.
		*
	...
	/** Determine the directions where to move an object at a given position to
		* go to de attrative point.
	...
	/** This function does a local optimisation by trying to move the object
		* defined by the rectangles Rects and with initial position PosX and PosY,
		*	so close at possible from the attractive point.
	...
	/** This is the heuristic used to make a placement of a given set of objects.
		...
		* @return						The function returns true if a placement has been
		*										found.
		*/
  bool HeuristicBL(RObj2D **objs,unsigned int nbobjs);
	...
	/** This is the heuristic used to make a placement of a given set of objects.
		*
		*	The heuristic place the first object in the middle of the possible
		* rectangle, and construct a solution by adding the other objects all
		* around.
		* @param objs 			The array of pointers to the objects to place.
		* @param nbobjs			The number of objects to place.
		* @return						The function returns true if a placement has been
